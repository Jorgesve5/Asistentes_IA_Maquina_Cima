                ...
                <div class="flex-1 bg-slate-100 overflow-hidden relative flex flex-col items-center justify-center">
                    @if($viewingManual->file_path)
                        @if($viewingManual->file_type === 'pdf')
                            <iframe src="{{ route('manual.view', $viewingManual->id) }}#toolbar=1&navpanes=0" class="w-full h-full border-none"></iframe>
                        @elseif($viewingManual->file_type === 'image')
                            <div class="w-full h-full p-6 flex items-center justify-center overflow-auto bg-slate-50">
                                <img src="{{ route('manual.view', $viewingManual->id) }}" alt="{{ $viewingManual->fileName }}" class="max-w-full max-h-full object-contain rounded-lg border border-slate-200 shadow-lg" />
                            </div>
                        @elseif($viewingManual->file_type === 'excel')
                            <div class="w-full h-full flex flex-col bg-white overflow-hidden">
                                ...
                                <div class="flex-1 overflow-auto"><div id="sheetjs-output-{{ $viewingManual->id }}" class="min-w-full"></div></div>
                            </div>
                            <script>
                            (function() {
                        ...
                        @elseif($viewingManual->file_type === 'word')
                            <iframe src="https://view.officeapps.live.com/op/embed.aspx?src={{ urlencode(route('manual.view', $viewingManual->id)) }}" class="w-full h-full border-none" loading="lazy"></iframe>
                        @else
                            <iframe src="https://docs.google.com/viewer?url={{ urlencode(route('manual.view', $viewingManual->id)) }}&embedded=true" class="w-full h-full border-none" loading="lazy"></iframe>
                        @endif
                    @else
